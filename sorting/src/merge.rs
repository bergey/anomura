...
fn merge_borrowed(a: &mut [i32], p: usize, q: usize, r: usize) {
    let mut i = p;
    let mut j = q;
...
fn merge_borrowed_helper(a: &mut [i32], p: usize, r: usize) {
    if r - p > 1 {
        let q = (r + p).div(2);
        merge_borrowed_helper(a, p, q);
        merge_borrowed_helper(a, q, r);
        merge_borrowed(a, p, q, r);
    }
}

fn merge_borrowed_sort(a: &mut [i32]) {
    let l = a.len();
    merge_borrowed_helper(a, 0, l);
}
...
mod tests {
    ...
    fn merge_mut_already_sorted() {
        ...
        merge_borrowed_sort(&mut sorted);
        assert_eq!(sorted, [1, 2, 3]);
    }
